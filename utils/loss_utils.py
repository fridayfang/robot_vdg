...
def l1_loss(network_output, gt, return_map=False):
    if not return_map:
...
def l2_loss(network_output, gt, return_map=False):
    if not return_map:
...
def ssim_noavg(img1, img2, mask=None, window_size=11, size_average=True):
    channel = img1.size(-3)
    window = create_window(window_size, channel)
    ...
    return _ssim_noavg(img1, img2, window, window_size, channel, size_average)

def _ssim_noavg(img1, img2, window, window_size, channel, size_average=True):
    mu1 = F.conv2d(img1, window, padding=window_size // 2, groups=channel)
    mu2 = F.conv2d(img2, window, padding=window_size // 2, groups=channel)
