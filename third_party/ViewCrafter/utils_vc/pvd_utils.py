...
def generate_traj_txt_my(c2ws_anchor,H,W,fs,c,phi, theta, r,frame,device,viz_traj=False, save_dir = None):
    # Initialize a camera.
    """
    The camera coordinate sysmte in COLMAP is right-down-forward
    Pytorch3D is left-up-forward
...
def world_point_to_kth_my(poses, points, k, device):
    # 将世界坐标系转到和第k个pose的相机坐标系一致,同时处理点云
    kth_pose = poses[k]
    inv_kth_pose = torch.inverse(kth_pose)
    # 给所有pose左成kth_w2c,将其都变到kth_pose的camera coordinate下
    ...
    return new_poses, new_points, kth_pose, inv_kth_pose
...
def world_point_to_obj_my(poses, points, k, r, elevation, device):
    ...
    poses, points, tranform1, transform1_inv = world_point_to_kth_my(poses, points, k, device)
    ...
    transform2 = pose_obj
    transform2_inv = inv_obj_pose
    transform_total = tranform1 @ transform2
    
    return new_poses, new_points, transform_total
