...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, 
        ...
        if shuffle:
            ...
            if scene_info.project_cameras is not None: 
                random.shuffle(scene_info.project_cameras)

        self.scene_info_train_cams = scene_info.train_cameras
        self.scene_info_all_cams = scene_info.all_cameras
        self.scene_info_train_indices = scene_info.train_indices

        self.cameras_extent = scene_info.nerf_normalization["radius"]

        for resolution_scale in resolution_scales:
            print("Loading Training Cameras")
            self.train_cameras[resolution_scale] = cameraList_from_camInfos(scene_info.train_cameras, resolution_scale, args) # 4032, 3024
            print("Loading Test Cameras")
            self.test_cameras[resolution_scale] = cameraList_from_camInfos(scene_info.test_cameras, resolution_scale, args)
            
            if scene_info.project_cameras is not None: 
                print("Loading Project Cameras")
                self.project_cameras[resolution_scale] = cameraList_from_camInfos(scene_info.project_cameras, resolution_scale, args)

        self.ply_path = os.path.join(self.model_path, "point_cloud", "iteration_"+str(self.loaded_iter), "point_cloud.ply") \
                            if self.loaded_iter else scene_info.ply_path
        if self.loaded_iter:
    ...
    def getProjectCameras(self, scale=1.0):
        return self.project_cameras[scale]
    
    def getAllCameras(self, scale=1.0):
        return self.all_cameras[scale]
